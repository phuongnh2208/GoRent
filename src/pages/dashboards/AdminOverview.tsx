  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Chào mừng quay lại, quản trị viên GoRent.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <motion.div 
            ...
            className="glass bg-white/40 p-6 rounded-3xl border border-white/20 shadow-xl shadow-maroon/5"
          >
            <div className="flex justify-between items-start mb-4">
               <div className="w-12 h-12 glass bg-white/60 rounded-2xl flex items-center justify-center border border-white">
            ...
            <p className="text-xs font-bold text-maroon/40 uppercase tracking-widest">{stat.name}</p>
            <p className="text-2xl font-bold tracking-tighter mt-1 text-maroon">{stat.value}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Recent Contracts */}
        <div className="lg:col-span-2 glass bg-white/40 rounded-3xl border border-white/20 shadow-xl shadow-maroon/5 p-8">
           <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-maroon">Hợp đồng gần đây</h3>
              <button className="text-xs font-bold text-maroon/40 hover:text-maroon transition-colors uppercase tracking-widest">Xem tất cả</button>
           </div>
             ...
             <table className="w-full text-left">
               <thead>
                 <tr className="border-b border-white/10">
                   <th className="pb-4 text-[10px] font-bold text-maroon opacity-40 uppercase tracking-widest">Khách hàng</th>
                   <th className="pb-4 text-[10px] font-bold text-maroon opacity-40 uppercase tracking-widest">Xe</th>
                   <th className="pb-4 text-[10px] font-bold text-maroon opacity-40 uppercase tracking-widest">Ngày thuê</th>
                   <th className="pb-4 text-[10px] font-bold text-maroon opacity-40 uppercase tracking-widest text-right">Trạng thái</th>
                 </tr>
               </thead>
               <tbody className="divide-y divide-white/10">
                 {contracts.slice(0, 5).map((contract) => (
                   <tr key={contract.id} className="group hover:bg-white/40 transition-colors">
                     <td className="py-4">
                        <p className="text-sm font-bold text-maroon">{contract.customerName}</p>
                        <p className="text-xs text-maroon/40">{contract.customerPhone}</p>
                     </td>
                     <td className="py-4">
                        <p className="text-sm font-medium text-maroon/80">{cars.find(c => c.id === contract.carId)?.name}</p>
                     </td>
                     <td className="py-4">
                        <p className="text-sm text-maroon/50">{contract.startDate} - {contract.endDate}</p>
                     </td>
        ...
        <div className="glass bg-white/40 rounded-3xl border border-white/20 shadow-xl shadow-maroon/5 p-8">
           ...
           <div className="space-y-6">
              {[
                ...
                { label: 'Đang thuê', count: contracts.filter(c => c.status === 'delivered').length, color: 'bg-maroon-light' },
                { label: 'Bảo dưỡng', count: cars.filter(c => c.status === 'maintenance').length, color: 'bg-yellow-500' },
              ].map(item => (
                <div key={item.label}>
                   <div className="flex justify-between items-center mb-2">
                     <span className="text-xs font-bold text-maroon/40 uppercase tracking-widest">{item.label}</span>
                     <span className="text-sm font-bold text-maroon">{item.count} xe</span>
                   </div>
